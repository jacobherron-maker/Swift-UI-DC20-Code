...
import type { RuleReferenceEntry } from '../../types/models';
...
function RuleDocument({ entry }: { entry: RuleReferenceEntry }) {
  return <article className="mx-auto max-w-4xl"><div className="mb-7 border-b border-white/10 pb-6"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.14em] ${kindColors[entry.kind] ?? kindColors.Rule}`}>{entry.kind}</span><span className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">{entry.section} • {entry.subsection}</span></div><h1 className="mt-4 text-4xl font-black leading-tight text-white">{entry.title}</h1><p className="mt-3 text-lg leading-7 text-violet-200">{entry.summary}</p><p className="mt-3 text-xs font-bold uppercase tracking-[0.16em] text-slate-500">{entry.page}</p></div><RichRuleText text={entry.text} />{entry.kind === 'Class' && entry.characterClass && <ClassTable className={entry.characterClass} />}</article>;
}
...
const RulesView: React.FC = () => {
  ...
  const filtered = reference.entries.filter((entry) => {
    const matchesSearch = !query || [entry.title, entry.summary, entry.text, entry.keywords, entry.page, entry.characterClass, entry.subclassName].some((value) => value?.toLowerCase().includes(query));
  ...
  return <div className="min-h-full bg-[radial-gradient(circle_at_top,#312e81_0%,#111827_38%,#020617_100%)] p-4 lg:p-7"><div className="mx-auto max-w-[1550px]"><header className="mb-5"><p className="text-xs font-black uppercase tracking-[0.3em] text-violet-300">475 curated reference documents</p><h1 className="mt-1 text-4xl font-black text-white">DC20 Rules Library</h1><p className="mt-2 max-w-3xl text-slate-400">Core rules, combat, general play, character creation, conditions, every class table, and standalone subclass documents—organized to match the Beta.</p></header>
    ...
    <div className="grid gap-4 lg:grid-cols-[360px_1fr]"><aside className="rounded-2xl border border-white/10 bg-slate-950/60 p-3 lg:h-[calc(100vh-310px)] lg:min-h-[620px] lg:overflow-auto"><div className="mb-2 flex items-center justify-between px-2 py-1"><h2 className="font-black text-violet-200">{query ? 'Search Results' : section}</h2><span className="rounded-full bg-slate-800 px-2 py-1 text-xs text-slate-400">{filtered.length}</span></div>{filtered.length === 0 ? <p className="p-4 text-sm text-slate-500">No rules match these filters.</p> : filtered.map((entry) => <button type="button" key={entry.id} onClick={() => setSelectedID(entry.id)} className={`mb-1 w-full rounded-xl p-3 text-left ${selected?.id === entry.id ? 'bg-violet-500/15 ring-1 ring-violet-400/40' : 'hover:bg-white/5'}`}><div className="flex items-start justify-between gap-2"><span className="font-bold text-slate-200">{entry.title}</span><span className={`shrink-0 rounded-full px-2 py-0.5 text-[9px] font-black uppercase ${kindColors[entry.kind] ?? kindColors.Rule}`}>{entry.kind}</span></div><div className="mt-1 text-xs text-slate-500">{entry.subsection} • {entry.page}</div><p className="mt-2 line-clamp-2 text-xs leading-5 text-slate-400">{entry.summary}</p></button>)}</aside><main className="rounded-2xl border border-white/10 bg-slate-900/75 p-6 lg:h-[calc(100vh-310px)] lg:min-h-[620px] lg:overflow-auto lg:p-9">{selected ? <RuleDocument entry={selected} /> : <div className="grid h-full place-items-center text-slate-500">Select a reference document.</div>}</main></div>
  </div></div>;
};
