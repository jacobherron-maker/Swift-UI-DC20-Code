...
function RuleDocument({ entry }: { entry: RuleReferenceEntry }) {
  return <article className="mx-auto max-w-4xl"><div className="mb-7 border-b border-white/10 pb-6"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.14em] ${kindColors[entry.kind] ?? kindColors.Rule}`}>{entry.kind}</span><span className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">{entry.section} • {entry.subsection}</span></div><h1 className="mt-4 break-words text-3xl font-black leading-tight text-white sm:text-4xl">{entry.title}</h1><p className="mt-3 text-lg leading-7 text-violet-200">{entry.summary}</p><p className="mt-3 text-xs font-bold uppercase tracking-[0.16em] text-slate-500">{entry.page}</p></div><RichRuleText text={entry.text} />{entry.kind === 'Class' && entry.characterClass && <ClassTable className={entry.characterClass} />}</article>;
}
