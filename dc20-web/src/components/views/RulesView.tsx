...
const sourceStatusMeta: Record<NonNullable<RuleReferenceEntry['sourceStatus']>, { icon: string; className: string }> = {
  'Beta source verified': { icon: '✓', className: 'border-emerald-400/20 bg-emerald-500/10 text-emerald-200' },
  'Supplemental source verified': { icon: '✓', className: 'border-emerald-400/20 bg-emerald-500/10 text-emerald-200' },
  'Verified source excerpt': { icon: '✓', className: 'border-sky-400/20 bg-sky-500/10 text-sky-200' },
  'Condensed source reference': { icon: '≈', className: 'border-amber-400/20 bg-amber-500/10 text-amber-200' },
  'Catalog source reference': { icon: '◇', className: 'border-cyan-400/20 bg-cyan-500/10 text-cyan-200' },
};
...
function RuleDocument({ entry, classReference, entries, onOpenRelated, onOpenSection, onOpenSubsection }: { entry: RuleReferenceEntry; classReference?: ClassReference; entries: RuleReferenceEntry[]; onOpenRelated: (entry: RuleReferenceEntry) => void; onOpenSection: (section: string) => void; onOpenSubsection: (section: string, subsection: string) => void }) {
  ...
  const status = entry.sourceStatus ?? 'Condensed source reference';
  const statusMeta = sourceStatusMeta[status];
  return <article className="mx-auto max-w-5xl"><nav aria-label="Rule breadcrumb" className="mb-5 flex flex-wrap items-center gap-2 text-xs font-bold text-slate-500"><button type="button" onClick={() => onOpenSection(entry.section)} className="hover:text-violet-200">Library</button><span aria-hidden="true">›</span><button type="button" onClick={() => onOpenSection(entry.section)} className="hover:text-violet-200">{entry.section}</button><span aria-hidden="true">›</span><button type="button" onClick={() => onOpenSubsection(entry.section, entry.subsection)} className="hover:text-violet-200">{entry.subsection}</button><span aria-hidden="true">›</span><span className="text-slate-300">{entry.title}</span></nav><div className="mb-7 border-b border-white/10 pb-6"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.14em] ${kindColors[entry.kind] ?? kindColors.Rule}`}>{entry.kind}</span><span className={`rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.12em] ${statusMeta.className}`}>{statusMeta.icon} {status}</span><span className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">{entry.section} • {entry.subsection}</span></div><h1 className="mt-4 break-words text-3xl font-black leading-tight text-white sm:text-4xl">{entry.title}</h1><p className="mt-3 text-lg leading-7 text-violet-200"><RuleAwareText text={entry.summary} /></p><div className="mt-4 flex flex-wrap gap-2 text-xs font-bold uppercase tracking-[0.12em] text-slate-400"><span className="rounded-lg border border-white/10 bg-slate-950/50 px-3 py-2">{entry.sourceDocument}</span><span className="rounded-lg border border-white/10 bg-slate-950/50 px-3 py-2">{entry.page}</span></div></div>
...
const RulesView: React.FC<{ preset?: 'all' | 'character-options'; focusRequest?: ContentFocusRequest | null; onFocusHandled?: () => void; returnContextLabel?: string; onReturnToContext?: () => void }> = ({ preset = 'all', focusRequest, onFocusHandled, returnContextLabel, onReturnToContext }) => {
  ...
  const kindEntries = query
    ? reference.entries.filter((entry) => preset === 'all' || characterOptionKinds.has(entry.kind))
    : sectionEntries;
  const kinds = Array.from(new Set(kindEntries.map((entry) => entry.kind))).sort();
  ...
  const libraryDescription = preset === 'character-options' ? 'Browse source-linked ancestries, Classes, Subclasses, talents, skills, trades, and languages in one focused index.' : 'Source-linked core rules, combat, general play, character creation, conditions, equipment, powers, Classes, and standalone Subclasses. Each document identifies its printed source, page, and verification level.';
  return <div className="min-h-full bg-[radial-gradient(circle_at_top,#312e81_0%,#111827_38%,#020617_100%)] p-4 lg:flex lg:h-full lg:min-h-0 lg:flex-col lg:overflow-visible lg:p-7"><div className="mx-auto max-w-[1550px] lg:flex lg:h-full lg:min-h-0 lg:w-full lg:flex-col"><header className="mb-5 lg:shrink-0">{onReturnToContext && <button type="button" onClick={onReturnToContext} className="mb-4 rounded-xl border border-violet-400/25 bg-violet-500/10 px-4 py-2 text-sm font-black text-violet-100">← {returnContextLabel ?? 'Return to previous screen'}</button>}<p className="text-xs font-black uppercase tracking-[0.3em] text-violet-300">{preset === 'character-options' ? `${filtered.length} character-building documents` : `${reference.entries.length} source-linked reference documents`}</p><h1 className="mt-1 text-3xl font-black text-white sm:text-4xl">{libraryTitle}</h1><p className="mt-2 max-w-4xl text-slate-400">{libraryDescription}</p>{preset === 'all' && <div aria-label="Source verification legend" className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-[10px] font-bold uppercase tracking-wider text-slate-500"><span><strong className="text-emerald-300">✓ Verified</strong> complete canonical text</span><span><strong className="text-sky-300">✓ Excerpt</strong> verified source passages</span><span><strong className="text-amber-300">≈ Condensed</strong> concise reference</span><span><strong className="text-cyan-300">◇ Catalog</strong> sourced item record</span></div>}</header>
