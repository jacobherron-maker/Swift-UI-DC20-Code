...
const RulesView: React.FC<{ preset?: 'all' | 'character-options'; focusRequest?: ContentFocusRequest | null; onFocusHandled?: () => void; returnContextLabel?: string; onReturnToContext?: () => void }> = ({ preset = 'all', focusRequest, onFocusHandled, returnContextLabel, onReturnToContext }) => {
  ...
  return <div className="min-h-full bg-[radial-gradient(circle_at_top,#312e81_0%,#111827_38%,#020617_100%)] p-4 lg:p-7"><div className="mx-auto max-w-[1550px]"><header className="mb-5">{onReturnToContext && <button type="button" onClick={onReturnToContext} className="mb-4 rounded-xl border border-violet-400/25 bg-violet-500/10 px-4 py-2 text-sm font-black text-violet-100">← {returnContextLabel ?? 'Return to previous screen'}</button>}<p className="text-xs font-black uppercase tracking-[0.3em] text-violet-300">{preset === 'character-options' ? `${filtered.length} character-building documents` : `${reference.entries.length} source-audited reference documents`}</p><h1 className="mt-1 text-3xl font-black text-white sm:text-4xl">{libraryTitle}</h1><p className="mt-2 max-w-4xl text-slate-400">{libraryDescription}</p></header>
    ...
    <div className="grid gap-4 lg:grid-cols-[360px_1fr]"><aside className="max-h-96 overflow-auto rounded-2xl border border-white/10 bg-slate-950/60 p-3 overscroll-contain lg:h-[calc(100vh-310px)] lg:max-h-none lg:min-h-[620px]"><div className="mb-2 flex items-center justify-between px-2 py-1"><h2 className="font-black text-violet-200">{query ? 'Search Results' : section}</h2><span className="rounded-full bg-slate-800 px-2 py-1 text-xs text-slate-400">{filtered.length}</span></div>{filtered.length === 0 ? <p className="p-4 text-sm text-slate-500">No rules match these filters.</p> : filtered.map((entry) => <button type="button" key={entry.id} onClick={() => openRelated(entry)} className={`mb-1 w-full rounded-xl p-3 text-left ${selected?.id === entry.id ? 'bg-violet-500/15 ring-1 ring-violet-400/40' : 'hover:bg-white/5'}`}><div className="flex items-start justify-between gap-2"><span className="font-bold text-slate-200">{entry.title}</span><span className={`shrink-0 rounded-full px-2 py-0.5 text-[9px] font-black uppercase ${kindColors[entry.kind] ?? kindColors.Rule}`}>{entry.kind}</span></div><div className="mt-1 text-xs text-slate-500">{entry.subsection} • {entry.page}</div><p className="mt-2 line-clamp-2 text-xs leading-5 text-slate-400">{entry.summary}</p></button>)}</aside><main className="rounded-2xl border border-white/10 bg-slate-900/75 p-4 sm:p-6 lg:h-[calc(100vh-310px)] lg:min-h-[620px] lg:overflow-auto lg:p-9"><div className="mb-5 flex gap-2"><button type="button" aria-label="Previous rule" disabled={ruleHistoryIndex <= 0} onClick={() => navigateRuleHistory(ruleHistoryIndex - 1)} className="rounded-lg bg-slate-950/60 px-3 py-2 text-sm text-slate-300 disabled:opacity-25">← Back</button><button type="button" aria-label="Next rule" disabled={ruleHistoryIndex < 0 || ruleHistoryIndex >= ruleHistory.length - 1} onClick={() => navigateRuleHistory(ruleHistoryIndex + 1)} className="rounded-lg bg-slate-950/60 px-3 py-2 text-sm text-slate-300 disabled:opacity-25">Forward →</button></div>{selected ? <RuleDocument entry={selected} classReference={selectedClass} entries={reference.entries} onOpenRelated={openRelated} onOpenSection={chooseSection} onOpenSubsection={(nextSection, nextSubsection) => { setSection(nextSection); setSubsection(nextSubsection); setKind('All Types'); setSearch(''); setSelectedID(null); }} /> : <div className="grid h-full place-items-center text-slate-500">Select a reference document.</div>}</main></div>
  </div></div>;
};
