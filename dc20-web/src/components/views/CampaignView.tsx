...
export default function CampaignView({ focusRequest, onFocusHandled }: { focusRequest?: ContentFocusRequest | null; onFocusHandled?: () => void }) {
  ...
  const addSharedVaultEntry = async (entry: GmVaultEntry) => {
    ...
    const claimedCount = party?.members.filter(({ character: memberCharacter }) => memberCharacter?.vaultEntries?.some(({ id }) => id === entry.id)).length ?? 0;
    if (entry.distribution?.quantityLimit && claimedCount >= entry.distribution.quantityLimit) {
      setNotice(`${entry.name} has reached its campaign claim limit.`);
      return;
    }
...
function CampaignVaultPanel({ party, character, onAdd }: { party: PartyCampaignSnapshot; character?: Character; onAdd: (entry: GmVaultEntry) => void }) {
  return <section className="rounded-2xl border border-fuchsia-400/20 bg-slate-900/70 p-4 sm:p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-fuchsia-300">Campaign Rewards</p><h2 className="mt-1 text-xl font-black text-white">Shared GM Vault</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-400">Your GM shared these options with this campaign. Review the complete rules, then add an eligible entry to your linked character. Items enter inventory unequipped.</p></div>{character ? <span className="rounded-full bg-violet-500/10 px-3 py-1 text-xs font-bold text-violet-200">Adding to {character.name}</span> : <span className="rounded-full bg-amber-500/10 px-3 py-1 text-xs font-bold text-amber-200">Choose a shared character first</span>}</div><div className="mt-5 grid gap-4 md:grid-cols-2">{party.vaultEntries.map((entry) => { const eligibility = character ? vaultEntryEligibility(character, entry) : { eligible: false, reason: 'Choose your shared character on the Party tab.' }; const added = Boolean(character?.vaultEntries?.some(({ id }) => id === entry.id)); const claimedCount = party.members.filter(({ character: memberCharacter }) => memberCharacter?.vaultEntries?.some(({ id }) => id === entry.id)).length; const claimLimitReached = Boolean(entry.distribution?.quantityLimit && claimedCount >= entry.distribution.quantityLimit && !added); const effects = vaultEffectSummary(entry.effects); return <article key={entry.id} className="rounded-xl border border-white/10 bg-slate-950/55 p-4"><div className="flex items-start justify-between gap-3"><div><span className="text-[10px] font-black uppercase tracking-wider text-fuchsia-300">{entry.kind} • v{entry.version ?? 1}</span><h3 className="mt-1 text-lg font-black text-slate-100">{entry.name}</h3></div>{added && <span className="rounded-full bg-emerald-500/10 px-2 py-1 text-[10px] font-black text-emerald-200">On sheet</span>}</div>{entry.summary && <p className="mt-2 text-sm text-slate-400">{entry.summary}</p>}{entry.distribution?.notes && <p className="mt-2 rounded-lg bg-fuchsia-500/10 p-2 text-xs text-fuchsia-100">{entry.distribution.notes}</p>}<div className="mt-2 flex flex-wrap gap-2">{entry.distribution?.mode === 'Assigned Characters' && <span className="rounded-full bg-sky-500/10 px-2 py-1 text-[10px] font-black text-sky-200">Assigned reward</span>}{entry.distribution?.quantityLimit && <span className="rounded-full bg-slate-800 px-2 py-1 text-[10px] font-black text-slate-300">{claimedCount}/{entry.distribution.quantityLimit} claimed</span>}{(entry.bundledEntries?.length ?? 0) > 0 && <span className="rounded-full bg-violet-500/10 px-2 py-1 text-[10px] font-black text-violet-200">Bundle: {entry.bundledEntries!.length + 1} entries</span>}</div><details className="group mt-3 rounded-lg border border-white/5 bg-white/[0.025] p-3"><summary className="cursor-pointer text-xs font-black text-violet-200"><span className="group-open:hidden">More details</span><span className="hidden group-open:inline">Less details</span></summary><p className="mt-3 whitespace-pre-wrap border-t border-white/5 pt-3 text-sm leading-6 text-slate-300">{entry.description || 'No additional description.'}</p>{effects.length > 0 && <div className="mt-3 flex flex-wrap gap-1.5">{effects.map((effect) => <span key={effect} className="rounded-full bg-violet-500/10 px-2 py-1 text-[10px] font-bold text-violet-200">{effect}</span>)}</div>}{entry.requirements.notes && <p className="mt-3 text-xs text-amber-200">{entry.requirements.notes}</p>}{(entry.bundledEntries?.length ?? 0) > 0 && <div className="mt-3 border-t border-white/5 pt-3"><p className="text-[10px] font-black uppercase text-violet-300">Also includes</p><p className="mt-1 text-xs text-slate-400">{entry.bundledEntries!.map(({ name, kind }) => `${name} (${kind})`).join(', ')}</p></div>}</details><button type="button" disabled={!eligibility.eligible || added || claimLimitReached} onClick={() => onAdd(entry)} className="mt-4 w-full rounded-lg bg-fuchsia-700 px-3 py-2 text-xs font-black text-white disabled:cursor-not-allowed disabled:bg-slate-800 disabled:text-slate-500">{added ? 'Already Added' : claimLimitReached ? 'Campaign claim limit reached' : eligibility.eligible ? `Add to ${character?.name}` : eligibility.reason}</button></article>; })}{party.vaultEntries.length === 0 && <div className="rounded-xl border border-dashed border-white/10 p-10 text-center text-slate-500 md:col-span-2">The GM has not shared any Vault content with this campaign yet.</div>}</div></section>;
}
...
function SharedInventoryEditor({ party, currentUserId, onCreate, onUpdate, onDelete }: { party: PartyCampaignSnapshot; currentUserId: string; onCreate: (item: PartyInventoryItem) => void; onUpdate: (item: PartyInventoryItem) => void; onDelete: (id: string) => void }) {
  ...
  return <div className="space-y-5"><section className="rounded-2xl border border-amber-400/20 bg-amber-950/15 p-4 sm:p-5"><p className="text-[10px] font-black uppercase tracking-[0.2em] text-amber-300">Party Stash</p><h2 className="mt-1 text-xl font-black text-white">Add a shared item</h2><p className="mt-2 text-xs leading-5 text-slate-500">Use this form for informal loot. GMs can send complete, source-linked equipment records from the Items & Equipment library.</p><div className="mt-4 grid gap-3 md:grid-cols-[minmax(0,0.7fr)_minmax(0,1.3fr)_auto]"><input value={name} onChange={(event) => setName(event.target.value)} className={inputClass} placeholder="Item name" aria-label="Shared item name" /><input value={description} onChange={(event) => setDescription(event.target.value)} className={inputClass} placeholder="Description, owner, location…" aria-label="Shared item description" /><button type="button" disabled={!name.trim()} onClick={create} className="rounded-xl bg-amber-700 px-5 py-2 font-black text-white disabled:opacity-40">Add Item</button></div></section><section className="rounded-2xl border border-white/10 bg-slate-900/70 p-4 sm:p-5"><div className="flex items-center justify-between"><h2 className="text-xl font-black text-white">Shared Inventory</h2><span className="text-xs font-bold text-slate-500">{party.inventory.reduce((sum, item) => sum + item.quantity, 0)} total</span></div><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{party.inventory.map((item) => {
    const gmControlled = Boolean(item.distributedByGM && party.role !== 'gm');
    const metadata = [item.itemSnapshot?.category, item.itemSnapshot?.subtype, item.ownerName && `Owner: ${item.ownerName}`, item.goldCost !== undefined && `${item.goldCost} gold each`, item.remainingUses !== undefined && `${item.remainingUses} ${item.itemSnapshot?.usageLabel?.toLowerCase() ?? 'uses'} left`].filter(Boolean);
    return <article key={`${item.id}-${item.updatedAt}`} className={`rounded-xl border p-4 ${item.distributedByGM ? 'border-sky-400/15 bg-sky-950/20' : 'border-white/10 bg-slate-950/55'}`}><div className="mb-2 flex flex-wrap gap-2">{item.distributedByGM && <span className="rounded-full bg-sky-500/10 px-2 py-1 text-[9px] font-black uppercase text-sky-200">GM Distributed</span>}{item.itemSnapshot && <span className="rounded-full bg-emerald-500/10 px-2 py-1 text-[9px] font-black uppercase text-emerald-200">Catalog Linked</span>}</div><input defaultValue={item.name} disabled={gmControlled} onBlur={(event) => onUpdate({ ...item, name: event.target.value })} className="w-full bg-transparent font-black text-slate-100 outline-none disabled:cursor-default" aria-label={`${item.name} name`} /><textarea defaultValue={item.description} disabled={gmControlled} onBlur={(event) => onUpdate({ ...item, description: event.target.value })} rows={3} className="mt-2 w-full resize-none bg-transparent text-sm leading-5 text-slate-400 outline-none disabled:cursor-default" aria-label={`${item.name} description`} />{metadata.length > 0 && <div className="mt-2 flex flex-wrap gap-1.5">{metadata.map((value) => <span key={String(value)} className="rounded bg-white/5 px-2 py-1 text-[10px] font-bold text-slate-400">{value}</span>)}</div>}{item.itemSnapshot?.sourcePage && <p className="mt-2 text-[10px] text-slate-600">{item.itemSnapshot.sourceDocument ? `${item.itemSnapshot.sourceDocument} • ` : ''}{item.itemSnapshot.sourcePage}</p>}<div className="mt-3 flex items-center justify-between"><div className="flex items-center gap-2"><button type="button" disabled={gmControlled} onClick={() => onUpdate({ ...item, quantity: Math.max(0, item.quantity - 1) })} className="h-8 w-8 rounded-lg bg-slate-800 font-black disabled:opacity-30">−</button><span className="min-w-8 text-center font-black text-amber-200">{item.quantity}</span><button type="button" disabled={gmControlled} onClick={() => onUpdate({ ...item, quantity: item.quantity + 1 })} className="h-8 w-8 rounded-lg bg-amber-700 font-black text-white disabled:opacity-30">+</button></div>{!gmControlled && <button type="button" onClick={() => { if (window.confirm(`Remove ${item.name} from the party inventory?`)) onDelete(item.id); }} className="text-xs font-bold text-red-300">Remove</button>}</div>{gmControlled && <p className="mt-2 text-[10px] text-slate-600">The GM manages this catalog-linked record.</p>}</article>;
  })}{party.inventory.length === 0 && <div className="rounded-xl border border-dashed border-white/10 p-10 text-center text-slate-500 md:col-span-2 xl:col-span-3">The shared inventory is empty.</div>}</div></section></div>;
}
