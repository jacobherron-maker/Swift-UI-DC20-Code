...
function InfoDetails({ summary, children }: { summary: React.ReactNode; children: React.ReactNode }) {
  return (
    <details className="group min-w-0 rounded-xl border border-white/10 bg-slate-950/45 p-4">
      <summary className="flex min-w-0 cursor-pointer list-none items-center justify-between gap-3 font-semibold text-slate-200">
        <div className="min-w-0 flex-1">{summary}</div><span className="shrink-0 text-xs text-violet-300 group-open:hidden">More</span><span className="hidden shrink-0 text-xs text-violet-300 group-open:inline">Less</span>
      </summary>
      <div className="mt-4 min-w-0 whitespace-pre-wrap break-words border-t border-white/5 pt-4 text-sm leading-6 text-slate-400">{React.Children.map(children, (child) => typeof child === 'string' ? <RuleAwareText text={child} /> : child)}</div>
    </details>
  );
}
...
function MasteryPicker({
  ...
  return (
    <div className="flex min-w-0 flex-wrap items-center justify-end gap-2">
      <select aria-label={`${label} mastery`} value={value} onChange={(event) => onChange(event.target.value as MasteryLevel)} className="min-w-0 max-w-full rounded-lg border border-slate-600 bg-slate-900 px-2 py-1 text-sm text-slate-200">
...
function LanguageFluencyPicker({
  ...
  return (
    <select
      aria-label={`${label} fluency`}
      value={value}
      disabled={isFree}
      onChange={(event) => onChange(event.target.value as LanguageFluency)}
      className="min-w-0 max-w-full rounded-lg border border-slate-600 bg-slate-900 px-2 py-1 text-sm text-slate-200 disabled:cursor-not-allowed disabled:opacity-70"
    >
...
const CharacterBuilderView: React.FC<{
  ...
  const expertise = reference ? (() => {
    const ancestry = ancestryExpertise(draft, reference.ancestryTraits);
    const classTrades = classTradeExpertise(draft);
    return {
      skills: ancestry.skills,
      trades: Object.fromEntries(Array.from(new Set([...Object.keys(ancestry.trades), ...Object.keys(classTrades)]))
        .map((trade) => [trade, Math.max(ancestry.trades[trade] ?? 0, classTrades[trade] ?? 0)])),
    };
  })() : { skills: {}, trades: {} };
  ...
  const navigateToStep = (step: BuilderStep) => {
    setCurrentStep(step);
    requestAnimationFrame(() => builderTopRef.current?.scrollIntoView({ block: 'start' }));
  };
  ...
  const featureChoiceGroups = (reference?.classes.flatMap((ownerReference) => ownerReference.choiceGroups
    .filter((group) => {
      ...
      return (nativeFeature || grantedFeature || talentFeature || referencedMulticlassChoice)
        ...
        && (group.id !== 'artificer.focusProperty' || (featureChoices['artificer.manaCore'] ?? []).includes('Focused'));
    })
  ...
  const psiKnightManeuver = featureChoices['psion.psiKnightManeuver'] ?? [];
  const psiKnightGrantedAttackNames = new Set(maneuvers.filter(({ category }) => category === 'Attack').map(({ name }) => name));
  const artificerInfusions = featureChoices['artificer.infusions'] ?? [];
  const artificerInfusionSpellCount = artificerInfusions.filter((name) => ['Spellcasting', 'Spell Potion', 'Spell Bomb'].includes(name)).length
    + Number(className === 'Artificer' && subclass === 'Apothecary') * 2;
  const artificerSpellInfusionSlots = [
    ...artificerInfusions.filter((name) => ['Spellcasting', 'Spell Potion', 'Spell Bomb'].includes(name)),
    ...(className === 'Artificer' && subclass === 'Apothecary' ? ['Spell Potion (Apothecary)', 'Spell Bomb (Apothecary)'] : []),
  ];
  const artificerInfusionSpells = featureChoices['artificer.infusionSpells'] ?? [];
  ...
  const setClass = (next: string) => {
    ...
    setInventoryItems((current) => current.filter(({ source }) => source !== 'startingEquipment'));
    setSpellListClass('');
  ...
  const setFeatureChoice = (group: ClassChoiceGroupReference, option: string) => {
    ...
    setFeatureChoices((current) => {
      ...
      if (group.id === 'artificer.manaCore' && !updated.includes('Focused')) delete next['artificer.focusProperty'];
      return next;
    });
  ...
  const adjustRepeatableFeatureChoice = (group: ClassChoiceGroupReference, option: string, direction: -1 | 1) => {
    setFeatureChoices((current) => {
      ...
      if (direction < 0) {
        ...
        const updated = selected.filter((_, candidate) => candidate !== index);
        const next = { ...current, [group.id]: updated };
        if (group.id === 'artificer.infusions') {
          const spellInfusionCount = updated.filter((name) => ['Spellcasting', 'Spell Potion', 'Spell Bomb'].includes(name)).length
            + Number(className === 'Artificer' && subclass === 'Apothecary') * 2;
          next['artificer.infusionSpells'] = (current['artificer.infusionSpells'] ?? []).slice(0, spellInfusionCount);
        }
        return next;
      }
  ...
  const canChooseSpellSource = hasSpells && spellAccessHasPublishedList && Boolean(spellAccessReference) && !spellAccessReference?.fixedSpellSource
    && !['Artificer', 'Bard', 'Psion', 'Spellblade', 'Summoner', 'Warlock'].includes(spellAccessClassName);
  ...
  const cantripOptions = ['Psion', 'Artificer'].includes(className)
  ...
  const validation = (() => {
    ...
    if (derived && derived.spellLimit > 0 && selectedSpells.length !== derived.spellLimit) issues.push(`Choose exactly ${derived.spellLimit} Class Table Spell${derived.spellLimit === 1 ? '' : 's'}.`);
    if (derived && derived.cantripLimit > 0 && selectedCantrips.length !== derived.cantripLimit) issues.push(`Choose exactly ${derived.cantripLimit} Class Table Cantrip${derived.cantripLimit === 1 ? '' : 's'}.`);
    if (derived && derived.maneuverLimit > 0 && selectedManeuvers.length !== derived.maneuverLimit) issues.push(`Choose exactly ${derived.maneuverLimit} Class Table Maneuver${derived.maneuverLimit === 1 ? '' : 's'}.`);
    if (className === 'Psion' && derived) {
      ...
      if (selectedCantrips.some((spell) => !cantripOptions.some(({ name: option }) => option === spell))) issues.push('Remove Cantrips that are no longer available to the Psion.');
    ...
    if (className === 'Artificer' && derived) {
      if (selectedSpells.some((spell) => !allowedSpells.some(({ name: option }) => option === spell))) issues.push('Remove Spells that are no longer on the Artificer Spell List.');
      if (selectedCantrips.some((spell) => !cantripOptions.some(({ name: option }) => option === spell))) issues.push('Remove Cantrips that are no longer on the Artificer Spell List.');
      if (selectedSpells.some((spell) => selectedCantrips.includes(spell))) issues.push('A Spell cannot fill both an Artificer Spell Known slot and an Artificer Cantrip Known slot.');
      if (artificerInfusionSpells.length !== artificerInfusionSpellCount || artificerInfusionSpells.some((spell) => !spell)) issues.push(`Choose ${artificerInfusionSpellCount} Infusion Spell${artificerInfusionSpellCount === 1 ? '' : 's'} for the selected spell-bearing Infusions.`);
      if (featureChoices['artificer.manaCore']?.[0] === 'Focused' && featureChoices['artificer.focusProperty']?.length !== 1) issues.push('Choose the Spell Focus Property granted by the Focused Mana Core.');
      if (subclass === 'Apothecary') {
        const unrestrictedBudget = Math.max(0, derived.tradePointBudget - 2);
        const nonApothecarySpent = Object.entries(tradeMasteries)
          .filter(([trade]) => !['Alchemy', 'Herbalism'].includes(trade))
          .reduce((sum, [, mastery]) => sum + masteryRank(mastery), 0);
        if (nonApothecarySpent > unrestrictedBudget) issues.push('The 2 Trade Points from Potions & Poisons can only be spent on Alchemy or Herbalism.');
      }
    }
    if (className === 'Psion' && subclass === 'Psi-Knight' && psiKnightManeuver.length !== 1) issues.push('Choose the additional Maneuver granted by Psionic Combatant.');
    ...
    if (!equipmentLoading && classReference) {
      const requiredStartingEquipment = [
        { group: 'arsenal' as const, singular: 'Arsenal item', plural: 'Arsenal items', limit: classReference.startingEquipment.arsenalCount },
        { group: 'armor' as const, singular: 'set of Armor', plural: 'sets of Armor', limit: 1 },
        { group: 'tools' as const, singular: 'Trade Tool choice', plural: 'Trade Tool choices', limit: classReference.startingEquipment.tradeToolCount },
      ];
      for (const { group, singular, plural, limit } of requiredStartingEquipment) {
        if (limit <= 0) continue;
        const eligibleIDs = new Set(startingEquipment[group].map(({ id }) => id));
        const selectedCount = inventoryItems.filter((entry) => entry.source === 'startingEquipment' && eligibleIDs.has(entry.equipmentID)).length;
        if (selectedCount !== limit) issues.push(`Choose ${limit} ${limit === 1 ? singular : plural} on the Equipment step (${selectedCount}/${limit}).`);
      }
    }
    return issues;
  })();
  ...
  return (
    <div ref={builderTopRef} className="character-builder min-h-full p-4 lg:p-7">
      <div className="mx-auto max-w-[1500px]">
        ...
        <main className={`${panelClass} min-h-[620px]`}>
          ...
          {currentStep === 'skills' && <section>
            ...
            <div className="min-w-0 space-y-5">{reference.skillGroups.map((group) => <div key={group.name} className="min-w-0"><h3 className="mb-2 text-sm font-black uppercase tracking-[0.16em] text-violet-300">{group.name} Skills</h3><div className="grid min-w-0 gap-2 lg:grid-cols-2">{group.options.map((skillName) => { const item = reference.skills.find(({ name: candidate }) => candidate === skillName)!; const value = skillMasteries[skillName] ?? 'Untrained'; const pointsAvailable = Math.max(0, (derived?.skillPointBudget ?? 0) - skillSpent + masteryRank(value)); return <InfoDetails key={skillName} summary={<div className="flex min-w-0 flex-1 flex-col items-stretch justify-between gap-2 sm:flex-row sm:items-center sm:gap-3"><span className="min-w-0 break-words">{skillName}</span><span className="min-w-0 self-end sm:self-auto" onClick={(event) => event.stopPropagation()}><MasteryPicker label={skillName} value={value} maximum={Math.min(5, skillMasteryMaximum + (expertise.skills[skillName] ?? 0))} bonus={expertise.skills[skillName] ?? 0} pointsAvailable={pointsAvailable} onChange={(next) => setSkillMasteries((current) => ({ ...current, [skillName]: next }))} /></span></div>}><p className="mb-2 text-xs font-bold uppercase tracking-wider text-violet-300">{item.attribute}</p>{item.description}</InfoDetails>; })}</div></div>)}
              {reference.tradeGroups.map((group) => <div key={group.name} className="min-w-0"><h3 className="mb-2 text-sm font-black uppercase tracking-[0.16em] text-fuchsia-300">{group.name} Trades</h3><div className="grid min-w-0 gap-2 lg:grid-cols-2">{group.options.map((tradeName) => { const item = reference.trades.find(({ name: candidate }) => candidate === tradeName)!; const value = tradeMasteries[tradeName] ?? 'Untrained'; const pointsAvailable = Math.max(0, (derived?.tradePointBudget ?? 0) - tradeSpent + masteryRank(value)); return <InfoDetails key={tradeName} summary={<div className="flex min-w-0 flex-1 flex-col items-stretch justify-between gap-2 sm:flex-row sm:items-center sm:gap-3"><span className="flex min-w-0 flex-wrap items-center gap-2 break-words">{tradeName}<span className="rounded-full bg-fuchsia-500/10 px-2 py-0.5 text-[10px] font-black uppercase tracking-wider text-fuchsia-300">{item.attribute}</span></span><span className="min-w-0 self-end sm:self-auto" onClick={(event) => event.stopPropagation()}><MasteryPicker label={tradeName} value={value} maximum={Math.min(5, tradeMasteryMaximum + Math.max(expertise.trades[tradeName] ?? 0, clericHasKnowledgeDomain && group.name === 'Knowledge' ? 1 : 0))} bonus={expertise.trades[tradeName] ?? 0} pointsAvailable={pointsAvailable} onChange={(next) => setTradeMasteries((current) => ({ ...current, [tradeName]: next }))} /></span></div>}><p className="mb-2 text-xs font-bold uppercase tracking-wider text-fuchsia-300">{item.attribute} • {item.tool}</p>{item.description}</InfoDetails>; })}</div></div>)}
              {reference.languageGroups.map((group) => <div key={group.name} className="min-w-0"><h3 className="mb-2 text-sm font-black uppercase tracking-[0.16em] text-sky-300">{group.name} Languages</h3><div className="grid min-w-0 gap-2 lg:grid-cols-2">{group.options.map((languageName) => { const item = reference.languages.find(({ name: candidate }) => candidate === languageName)!; const value = effectiveLanguageFluencies[languageName] ?? 'Untrained'; const paidValue = languageFluencies[languageName] ?? 'Untrained'; const isClassGranted = grantedLanguages.includes(languageName); const isSorcererIncrease = sorcererLanguage === languageName; const isFree = languageName === 'Common' || isClassGranted; const pointsAvailable = Math.max(0, (derived?.languagePointBudget ?? 0) - languageSpent + (isFree ? 0 : fluencyRank(paidValue))); const classGrantName = className === 'Rogue' ? 'Cypher Speech' : className === 'Warlock' ? 'Alien Comprehension' : 'Class Feature'; return <InfoDetails key={languageName} summary={<div className="flex min-w-0 flex-1 flex-col items-stretch justify-between gap-2 sm:flex-row sm:items-center sm:gap-3"><span className="min-w-0 break-words">{languageName}{isFree && <span className="ml-2 text-xs text-emerald-300">Free • Fluent{isClassGranted ? ` • ${classGrantName}` : ''}</span>}{isSorcererIncrease && <span className="ml-2 text-xs text-fuchsia-300">+1 Fluency Stage • Sorcerer Appearance</span>}</span><span className="min-w-0 self-end sm:self-auto" onClick={(event) => event.stopPropagation()}><LanguageFluencyPicker label={languageName} value={isFree ? 'Fluent' : value} pointsAvailable={pointsAvailable} isFree={isFree} onChange={(next) => setLanguageFluencies((current) => ({ ...current, [languageName]: isSorcererIncrease ? LANGUAGE_FLUENCIES[Math.max(0, fluencyRank(next) - 1)] : next }))} /></span></div>}><p className="mb-2 text-xs font-bold uppercase tracking-wider text-sky-300">Typical speakers: {item.typicalSpeakers}</p><p className="mb-2 text-xs text-slate-500">Limited speakers make a Language Check when precise understanding or communication matters. Fluent speakers read, write, and speak without that check.</p>{item.description}</InfoDetails>; })}</div></div>)}
            </div>
          </section>}
