...
const CharacterBuilderView: React.FC<{
  ...
  const validation = (() => {
    ...
    if (className === 'Artificer' && derived) {
      if (selectedSpells.length !== derived.spellLimit) issues.push(`Choose exactly ${derived.spellLimit} Artificer Spells after any Spell-to-Infusion exchanges.`);
      if (selectedCantrips.length !== derived.cantripLimit) issues.push(`Choose exactly ${derived.cantripLimit} Artificer Cantrips.`);
