...
const CustomizeDialog: React.FC<CustomizeDialogProps> = ({ onClose }) => {
  ...
  return <div role="dialog" aria-modal="true" aria-labelledby="customize-heading" className="fixed inset-0 z-50 grid place-items-center bg-black/75 p-2 backdrop-blur-sm sm:p-4" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div className="flex max-h-[calc(100dvh-1rem)] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-white/10 bg-slate-950 shadow-2xl sm:max-h-[92dvh] sm:rounded-3xl">
      <header className="flex items-start justify-between gap-3 border-b border-white/10 p-4 sm:gap-4 sm:p-6">
        <div><p className="theme-accent-text text-xs font-black uppercase tracking-[0.25em]">Appearance & backups</p><h1 id="customize-heading" className="mt-1 text-2xl font-black text-white sm:text-3xl">Customize DC20 Hub</h1><p className="mt-2 max-w-2xl text-sm text-slate-400">Choose a curated class palette. Changes apply immediately and stay selected on this browser.</p></div>
        <button type="button" onClick={onClose} aria-label="Close customization" className="grid h-11 w-11 shrink-0 place-items-center rounded-full border border-white/10 bg-white/5 font-black text-slate-300 hover:bg-white/10">✕</button>
      </header>

      <div className="grid min-h-0 flex-1 overflow-auto lg:grid-cols-[1fr_310px]">
        <section className="grid content-start gap-3 p-4 sm:grid-cols-2 sm:p-6 xl:grid-cols-3">
        ...
        <aside className="border-t border-white/10 p-4 sm:p-6 lg:border-l lg:border-t-0" style={{ background: `linear-gradient(160deg, ${selected.backgroundSecondary}, ${selected.background})` }}>
