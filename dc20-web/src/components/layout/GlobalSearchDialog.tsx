...
export default function GlobalSearchDialog({ onClose, onNavigate }: { onClose: () => void; onNavigate: (target: Omit<ContentFocusRequest, 'key'>) => void }) {
  ...
  const allResults = useMemo<SearchResult[]>(() => {
    ...
    const monsterResults = [...sourceMonsters, ...campaignData.customMonsters].map((monster) => ({ key: `monster:${monster.id}`, title: monster.name, detail: `Level ${monster.level} ${monster.type} ${monster.role} • ${monster.creatureType}`, category: campaignData.customMonsters.some(({ id }) => id === monster.id) ? 'Homebrew' : 'Monster', keywords: `${monster.descriptionText} ${monster.tactics} ${monster.lore} ${monster.abilities.map(({ name, details }) => `${name} ${details}`).join(' ')}`, target: { kind: 'monster' as const, id: monster.id, name: monster.name } }));
