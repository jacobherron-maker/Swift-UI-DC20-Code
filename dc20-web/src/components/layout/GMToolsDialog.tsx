...
type GMTool = 'dice' | 'initiative' | 'calculator' | 'conditions';
...
const tools: Array<{ id: GMTool | 'rules'; icon: string; title: string; description: string }> = [
  ...
  { id: 'calculator', icon: '⌗', title: 'Quick Calculator', description: 'Resolve target numbers and check margins at the table.' },
...
function CalculatorTool() {
  ...
  const needed = Math.max(1, difficulty - modifier);
  const total = roll + modifier;
  const margin = total - difficulty;
  return <div className="mx-auto max-w-2xl p-5 sm:p-8"><div className="grid gap-4 sm:grid-cols-3">{[['Target DC', difficulty, setDifficulty], ['Check Modifier', modifier, setModifier], ['D20 Result', roll, setRoll]].map(([label, value, setter]) => <label key={String(label)} className="rounded-xl border border-white/10 bg-slate-900/70 p-4 text-[10px] font-black uppercase tracking-wider text-slate-500">{String(label)}<input type="number" value={Number(value)} onChange={(event) => (setter as (value: number) => void)(Number(event.target.value))} className="mt-2 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xl font-black text-white outline-none focus:border-violet-400" /></label>)}</div><div className="mt-5 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl border border-violet-400/20 bg-violet-500/10 p-5"><div className="text-xs font-black uppercase tracking-wider text-violet-300">Needed on the d20</div><div className="mt-2 text-4xl font-black text-white">{needed > 20 ? 'Impossible' : `${needed}+`}</div></div><div className={`rounded-2xl border p-5 ${margin >= 0 ? 'border-emerald-400/20 bg-emerald-500/10' : 'border-rose-400/20 bg-rose-500/10'}`}><div className={`text-xs font-black uppercase tracking-wider ${margin >= 0 ? 'text-emerald-300' : 'text-rose-300'}`}>Entered result</div><div className="mt-2 text-4xl font-black text-white">{total}</div><p className="mt-1 text-sm text-slate-400">{margin >= 0 ? `Success by ${margin}` : `Failure by ${Math.abs(margin)}`}</p></div></div><p className="mt-5 rounded-xl bg-slate-900/60 p-4 text-sm leading-6 text-slate-400">This is a quick table aid. Advantage, Disadvantage, Heavy Hits, and other situational effects still use their normal DC20 rules.</p></div>;
}
