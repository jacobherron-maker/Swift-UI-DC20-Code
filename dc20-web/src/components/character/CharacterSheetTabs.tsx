...
function CustomItemActions({ item, onSave, onDeletePermanently, onRemoveFromCharacter }: {
  item: EquipmentCatalogItem;
  onSave: (values: { name: string; summary: string; mechanics: string }) => void;
  ...
  const resetDraft = () => { setName(item.name); setHighlight(item.summary); setDescription(item.mechanics); };
  const save = () => {
    if (!name.trim()) return;
    onSave({ name: name.trim(), summary: highlight.trim(), mechanics: description.trim() });
    setIsEditing(false);
  };
