...
import { removeVaultEntryFromCharacter, vaultEffectSummary } from '../../utils/vaultRules';
import { FAMILIAR_RULES, SUMMON_RULES } from '../../utils/companionRules';
...
function MoreDetails({ title, subtitle, children }: { title: string; subtitle?: string; children: React.ReactNode }) {
  ...
  return <details open={open} onToggle={(event) => setOpen(event.currentTarget.open)} className="rounded-xl border border-white/10 bg-slate-950/45 p-4"><summary className="flex cursor-pointer list-none items-start justify-between gap-3"><span><span className="font-black text-slate-200">{title}</span>{subtitle && <span className="mt-1 block text-xs text-slate-500">{subtitle}</span>}</span><span className="text-xs font-bold text-violet-300">{open ? 'Less' : 'More'}</span></summary><div className="mt-4 whitespace-pre-wrap border-t border-white/5 pt-4 text-sm leading-6 text-slate-400">{React.Children.map(children, (child) => typeof child === 'string' ? <RuleAwareText text={child} /> : child)}</div></details>;
}
...
function CheckReferenceCard({ name, metadata, modifier, description, tone, supplemental, adjustmentLabel, onRoll }: {
  name: string;
  metadata: string;
  modifier: number;
  description: string;
  ...
  const colors = tone === 'fuchsia'
    ? { border: 'hover:border-fuchsia-400/40', value: 'text-fuchsia-200', detail: 'text-fuchsia-200' }
    : { border: 'hover:border-violet-400/40', value: 'text-violet-200', detail: 'text-violet-200' };
  return <div className="overflow-hidden rounded-xl border border-white/10 bg-slate-950/45">
    <button type="button" onClick={onRoll} aria-label={`Roll ${name}`} className={`flex w-full items-center justify-between gap-3 p-3 text-left transition hover:bg-white/[0.035] ${colors.border}`}>
      <span className="min-w-0"><span className="font-bold text-slate-200">{name}</span><span className="ml-2 text-xs text-slate-500">{metadata}</span>{supplemental}</span>
      <span className="shrink-0 text-right"><span className={`block text-lg font-black ${colors.value}`}>{modifier >= 0 ? '+' : ''}{modifier}</span><span className="block text-[9px] font-black uppercase tracking-wider text-slate-600">Roll</span>{adjustmentLabel}</span>
    </button>
    <details className="group border-t border-white/5">
      <summary className={`flex cursor-pointer list-none items-center justify-between gap-3 px-3 py-2 text-xs font-black ${colors.detail} hover:bg-white/[0.035]`}><span>What {name} covers</span><span><span className="group-open:hidden">More</span><span className="hidden group-open:inline">Less</span></span></summary>
      <div className="border-t border-white/5 px-3 py-3 text-xs leading-5 text-slate-400"><RuleAwareText text={description} /></div>
    </details>
  </div>;
}
...
export function CharacterRestControls({ character, onChange }: { character: Character; onChange: (character: Character) => void }) {
  ...
  const rest = (type: 'Quick' | 'Short' | 'Long') => {
    ...
    const restoredVaultCharges = (next.vaultEntries ?? []).reduce((sum, entry) => {
      const previous = before.vaultEntries?.find(({ id }) => id === entry.id);
      return sum + Math.max(0, (entry.remainingCharges ?? 0) - (previous?.remainingCharges ?? 0));
    }, 0);
    ...
    if (restoredVaultCharges > 0) details.push(`${restoredVaultCharges} Vault ${restoredVaultCharges === 1 ? 'charge' : 'charges'} restored`);
...
function MonkUnarmedAttackCard({ character, modifier, adjustment, damageBonus, impactBonus, prone, onChange, onRoll }: {
  ...
  const rollUnarmedAttack = (flurry: boolean) => {
    ...
    if (scorpionEnhancement) enhancementNotes.push(`Scorpion: +1 damage; Physical Save DC ${character.saveDC ?? 10 + character.primeModifier + character.combatMastery}, Failure makes the target Impaired on its next Physical Check before the end of your next turn`);
    if (internalDamageSP > 0) enhancementNotes.push(`Internal Damage: Repeated Physical Save DC ${character.saveDC ?? 10 + character.primeModifier + character.combatMastery}; Failure causes Impaired for 1 minute and ${internalDamageSP} True damage at the start of each turn`);
...
function EquippedRulesSummary({ modifiers }: { modifiers: ReturnType<typeof equippedCombatModifiers> }) {
  ...
  return <>{reductions.length > 0 && <div className="mt-3 rounded-lg border border-sky-400/15 bg-sky-500/5 p-2 text-xs text-sky-100"><strong>Equipped Damage Reduction:</strong> {reductions.join(' • ')} <span className="text-sky-300/70">(Resistance (Half); bypassed by Heavy and Critical Hits)</span></div>}{modifiers.immuneToFlanking && <div className="mt-3 rounded-lg border border-amber-400/15 bg-amber-500/5 p-2 text-xs text-amber-100"><strong>{modifiers.flankingImmunitySource}:</strong> immune to Flanking{modifiers.flankingImmunitySource === 'Two Shields' ? '; only one Shield’s defense bonuses apply at a time.' : '.'}</div>}{modifiers.mountedShieldDefense && <div className="mt-3 rounded-lg border border-emerald-400/15 bg-emerald-500/5 p-2 text-xs text-emerald-100"><strong>Mounted Shield:</strong> your Mount also gains +{modifiers.mountedShieldDefense.physicalDefense} PD and +{modifiers.mountedShieldDefense.areaDefense} AD.</div>}{contextual.length > 0 && <div className="mt-3 rounded-lg border border-fuchsia-400/15 bg-fuchsia-500/5 p-2 text-xs text-fuchsia-100"><strong>Focus Properties:</strong> {contextual.join(' • ')}</div>}{modifiers.senses.length > 0 && <div className="mt-3 rounded-lg border border-violet-400/15 bg-violet-500/5 p-2 text-xs text-violet-100"><strong>Active Senses:</strong> {modifiers.senses.join(' • ')}</div>}{modifiers.conditionSaveAdvantages.length > 0 && <div className="mt-3 rounded-lg border border-emerald-400/15 bg-emerald-500/5 p-2 text-xs text-emerald-100"><strong>Magic Item Save Advantages:</strong> ADV on Saves against {modifiers.conditionSaveAdvantages.join(' and ')}.</div>}{modifiers.conditionalRules.length > 0 && <div className="mt-3 rounded-lg border border-amber-400/15 bg-amber-500/5 p-3 text-xs leading-5 text-amber-100"><strong>Active Special Rules:</strong><ul className="mt-1 list-disc space-y-1 pl-5">{modifiers.conditionalRules.map((rule) => <li key={rule}>{rule}</li>)}</ul></div>}</>;
}
...
function ChecksTab({ character, reference, equipmentCatalog, equipmentModifiers, selectedTraits, onRoll }: {
  ...
  return <div className="space-y-5">
    ...
    <section>
      <button type="button" onClick={() => setSkillsOpen((open) => !open)} aria-expanded={skillsOpen} className="mb-3 flex w-full items-center justify-between rounded-xl border border-violet-400/10 bg-slate-950/55 p-4 font-black text-violet-200 transition hover:border-violet-400/30 hover:bg-violet-500/[0.06]"><span>Skills</span><span>{skillsOpen ? 'Collapse' : 'Expand'}</span></button>
      {skillsOpen && <div className="space-y-5">
        {meditationSkill && <div className="rounded-xl border border-violet-400/20 bg-violet-500/10 p-3 text-xs text-violet-100"><strong>Meditation:</strong> {meditationSkill} Mastery is increased by 1, up to your Skill Mastery Cap, until your next Short or Long Rest.</div>}
        {(reference?.skillGroups ?? []).map((group) => <div key={group.name}>
          ...
          <div className="grid gap-2 md:grid-cols-2">{group.options.map((name) => {
            ...
            return <CheckReferenceCard
              ...
              tone="violet"
              supplemental={<>{wildForm.active && wildForm.skillMasteries.includes(name) && <span className="ml-2 text-[10px] font-black uppercase text-emerald-300">Skillful</span>}{meditationSkill === name && <span className="ml-2 text-[10px] font-black uppercase text-violet-300">Meditation</span>}</>}
              adjustmentLabel={adjustment > 0 ? <span className="block text-[9px] font-bold text-emerald-300">ADV active</span> : adjustment < 0 ? <span className="block text-[9px] font-bold text-rose-300">Gear DisADV</span> : undefined}
              onRoll={() => onRoll(`${name} Check`, modifier, adjustment)}
            />;
          })}</div>
        </div>)}
      </div>}
    ...
    <section>
      <button type="button" onClick={() => setTradesOpen((open) => !open)} aria-expanded={tradesOpen} className="mb-3 flex w-full items-center justify-between rounded-xl border border-fuchsia-400/10 bg-slate-950/55 p-4 font-black text-fuchsia-200 transition hover:border-fuchsia-400/30 hover:bg-fuchsia-500/[0.06]"><span>Trades</span><span>{tradesOpen ? 'Collapse' : 'Expand'}</span></button>
      {tradesOpen && <div className="space-y-5">{(reference?.tradeGroups ?? []).map((group) => <div key={group.name}>
        ...
        <div className="grid gap-2 md:grid-cols-2">{group.options.map((name) => {
          ...
          const toolName = trade?.tool?.replace(/\s+required$/i, '') ?? '';
          return <CheckReferenceCard
            key={name}
            name={name}
            ...
            tone="fuchsia"
            supplemental={toolRequired && <span className={`mt-1 block text-[10px] font-bold ${toolReady ? 'text-emerald-300' : 'text-amber-300'}`}>{toolReady ? `${toolName} ready` : `${toolName} required for activities`}</span>}
            adjustmentLabel={adjustment < 0 ? <span className="block text-[9px] font-bold text-rose-300">Gear DisADV</span> : undefined}
            onRoll={() => onRoll(`${name} Trade Check`, modifier, adjustment)}
          />;
        })}</div>
      </div>)}</div>}
    </section>
...
function CombatTab({ character, training, modifiers, equipmentCatalog, knownSpells, knownManeuvers, grantedSpells, grantedManeuvers, ancestryGrantedSpells, selectedTraits, onChange, onRoll }: {
  ...
  const baseCheck = character.primeModifier + character.combatMastery + (wildForm.active ? 0 : modifiers.allCheckBonus);
  const martialCheck = baseCheck + (wildForm.active ? 0 : modifiers.martialCheckBonus);
  ...
  const activeResistances = wildForm.active
    ...
    : uniqueStrings([...effects.resistances, ...ancestryMechanics.resistances, ...modifiers.resistances]);
  const activeImmunities = wildForm.active ? [] : uniqueStrings(modifiers.immunities);
  const referenceStats: Array<[string, string | number, number | null]> = [
    ...
    ['Class Save DC', wildForm.active ? 10 + character.primeModifier + character.combatMastery : character.saveDC ?? 10 + character.primeModifier + character.combatMastery + modifiers.saveDCBonus, null],
    ...(!wildForm.active ? [
  ...
  return <div className="space-y-5">
    <section className={panelClass}>
      ...
      {activeImmunities.length > 0 && <div className="mt-3 rounded-lg border border-emerald-400/15 bg-emerald-500/5 p-2 text-xs text-emerald-100"><strong>Active Immunities:</strong> {activeImmunities.join(' • ')}</div>}
...
function FeaturesTab({ character, classReference, reference, selectedTraits, training, readOnly, onChange }: {
  ...
  const talents = Object.entries((build?.selectedTalents ?? []).reduce<Record<string, number>>(
  ...
  const vaultFeatures = character.vaultEntries ?? [];
  const setVaultCharges = (entryID: string, value: number) => onChange({
    vaultEntries: vaultFeatures.map((entry) => entry.id === entryID
      ? { ...entry, remainingCharges: Math.min(entry.charges ?? 0, Math.max(0, value)) }
      : entry),
  });

  return <div className="grid gap-5 lg:grid-cols-3">
    <section className={`${panelClass} lg:col-span-3`}>
      <SectionHeading eyebrow="Training" title="Combat Training" tone="text-amber-300" />
      <div className="mt-3 flex flex-wrap gap-2">{training.categories.length === 0
        ? <span className="rounded-full bg-slate-800 px-3 py-1.5 text-xs font-bold text-slate-400">No combat training recorded</span>
        : training.categories.map((entry) => <span key={entry} className="rounded-full border border-amber-400/20 bg-amber-500/10 px-3 py-1.5 text-xs font-black text-amber-100">{entry}</span>)}</div>
      {classReference?.pathDetails && <div className="mt-3"><MoreDetails title="Class Path & Training Rules" subtitle={character.class}>{classReference.pathDetails}</MoreDetails></div>}
    </section>
    <section>
      <h2 className="mb-3 font-black text-violet-200">Class Features</h2>
      <div className="space-y-3">
        {classFeatures.map((entry) => <div key={entry.level}><h3 className="mb-2 text-xs font-black uppercase tracking-wider text-slate-500">Level {entry.level}</h3><div className="space-y-2">{entry.features.map((feature) => <MoreDetails key={`${entry.level}-${feature.name}`} title={feature.name}>{featureDescription(feature.name, feature.description)}</MoreDetails>)}</div></div>)}
        {character.subclass && (classReference?.subclassFeatures[character.subclass] ?? []).filter((feature) => feature.level === undefined || feature.level <= character.level).map((feature) => <MoreDetails key={`subclass-${feature.name}`} title={feature.name} subtitle={`${character.subclass}${feature.level !== undefined ? ` • Level ${feature.level}` : ''}`}>{featureDescription(feature.name, feature.description)}</MoreDetails>)}
        {multiclassFeatures.length > 0 && <div><h3 className="mb-2 mt-4 text-xs font-black uppercase tracking-wider text-cyan-300">Multiclass Features</h3><div className="space-y-2">{multiclassFeatures.map((feature) => <MoreDetails key={`${feature.className}-${feature.subclass ?? ''}-${feature.name}`} title={feature.name} subtitle={`${feature.className}${feature.subclass ? ` • ${feature.subclass}` : ''} • Level ${feature.level} • ${feature.source}`}>{featureDescription(feature.name, feature.description, feature.className)}</MoreDetails>)}</div></div>}
      </div>
    </section>
    <section>
      <h2 className="mb-3 font-black text-fuchsia-200">Talents</h2>
      <div className="space-y-2">{talents.length === 0 ? <p className="text-slate-500">No talents selected.</p> : talents.map(([name, count]) => { const talent = reference ? talentByName(reference, name) : undefined; return <MoreDetails key={name} title={`${name}${count > 1 ? ` ×${count}` : ''}`} subtitle={talent ? `${talent.category}${talent.className ? ` • ${talent.className}` : ''}` : undefined}>{featureDescription(name, talent?.description ?? 'Talent details are unavailable.', talent?.className)}</MoreDetails>; })}</div>
    </section>
    <section>
      <h2 className="mb-3 font-black text-emerald-200">Ancestry Traits</h2>
      <div className="space-y-2">{selectedTraits.length === 0 ? <p className="text-slate-500">No ancestry traits selected.</p> : selectedTraits.map((trait) => {
        ...
        return <MoreDetails key={trait.id} title={`${trait.name}${count > 1 ? ` ×${count}` : ''}`} subtitle={`${trait.ancestry} • ${trait.category} • ${trait.cost > 0 ? '+' : ''}${trait.cost} AP each • ${source.title}, p. ${source.page}`}><div className="mb-3 flex flex-wrap gap-1.5">{tags.map((tag) => <span key={tag} className="rounded-full bg-emerald-500/10 px-2 py-1 text-[10px] font-black uppercase tracking-wider text-emerald-200">{tag}</span>)}</div>{trait.description}{build?.ancestryTraitChoices[trait.id]?.filter(Boolean).length ? `\n\nChoice${(build.ancestryTraitChoices[trait.id]?.filter(Boolean).length ?? 0) > 1 ? 's' : ''}: ${build.ancestryTraitChoices[trait.id].filter(Boolean).join(', ')}` : ''}</MoreDetails>;
      })}</div>
    </section>
    {vaultFeatures.length > 0 && <section className="lg:col-span-3">
      <h2 className="mb-1 font-black text-cyan-200">GM Vault Content</h2>
      <p className="mb-3 text-sm text-slate-500">These are self-contained copies accepted from a campaign. Removing one does not change the GM’s original.</p>
      <div className="grid gap-2 md:grid-cols-2">{vaultFeatures.map((entry) => {
        const effects = vaultEffectSummary(entry.effects);
        const maximumCharges = entry.charges ?? 0;
        const remainingCharges = Math.min(maximumCharges, Math.max(0, entry.remainingCharges ?? maximumCharges));
        return <MoreDetails key={entry.id} title={entry.name} subtitle={`${entry.kind} • Shared by your GM`}>
          <p>{entry.description}</p>
          {maximumCharges > 0 && <div className="mt-3 rounded-xl border border-fuchsia-400/15 bg-fuchsia-500/10 p-3"><div className="flex flex-wrap items-center justify-between gap-3"><span><span className="block text-[10px] font-black uppercase tracking-wider text-fuchsia-300">Charges / Uses</span><span className="text-lg font-black text-fuchsia-100">{remainingCharges} / {maximumCharges}</span><span className="ml-2 text-xs text-slate-500">• {entry.recharge ?? 'Long Rest'}</span></span>{!readOnly && <span className="flex gap-2"><button type="button" disabled={remainingCharges <= 0} onClick={() => setVaultCharges(entry.id, remainingCharges - 1)} className="rounded-lg bg-fuchsia-700 px-3 py-2 text-xs font-black text-white disabled:cursor-not-allowed disabled:opacity-35">Spend 1</button><button type="button" disabled={remainingCharges >= maximumCharges} onClick={() => setVaultCharges(entry.id, remainingCharges + 1)} className="rounded-lg bg-slate-700 px-3 py-2 text-xs font-black text-white disabled:cursor-not-allowed disabled:opacity-35">Restore 1</button></span>}</div></div>}
          {(entry.grantedSpells?.length ?? 0) > 0 && <div className="mt-3"><p className="text-[10px] font-black uppercase tracking-wider text-cyan-300">Connected Spells</p><div className="mt-2 flex flex-wrap gap-1.5">{entry.grantedSpells?.map((spell) => <span key={spell.id} className="rounded-full bg-cyan-500/10 px-2.5 py-1 text-xs font-bold text-cyan-100">{spell.name}</span>)}</div></div>}
          {effects.length > 0 && <div className="mt-3 flex flex-wrap gap-1.5">{effects.map((effect) => <span key={effect} className="rounded-full bg-cyan-500/10 px-2 py-1 text-[10px] font-bold text-cyan-200">{effect}</span>)}</div>}
          {entry.requirements.notes && <p className="mt-3 text-xs text-amber-200">Requirement notes: {entry.requirements.notes}</p>}
          {!readOnly && <button type="button" onClick={() => { if (window.confirm(`Remove “${entry.name}” from this character?`)) onChange(removeVaultEntryFromCharacter(character, entry.id)); }} className="mt-4 rounded-lg bg-red-500/10 px-3 py-2 text-xs font-black text-red-300 hover:bg-red-500/20">Remove from Character</button>}
        </MoreDetails>;
      })}</div>
    </section>}
  </div>;
...
/** Inline edit/delete controls for one custom item's inventory row. Deleting never saves a pending edit first. */
function CustomItemActions({ item, propertyOptions, onSave, onDeletePermanently, onRemoveFromCharacter }: {
  item: EquipmentCatalogItem;
  propertyOptions: string[];
  onSave: (values: { name: string; summary: string; mechanics: string; category: EquipmentCategory; slot: EquipmentSlot; properties: string[] }) => void;
  onDeletePermanently: () => void;
  onRemoveFromCharacter: () => void;
}) {
  const routedEffectOptions = Object.keys(ROUTED_SHEET_EFFECTS);
  const [isEditing, setIsEditing] = useState(false);
  const [name, setName] = useState(item.name);
  const [highlight, setHighlight] = useState(item.summary);
  const [description, setDescription] = useState(item.mechanics);
  const [itemCategory, setItemCategory] = useState<EquipmentCategory>(item.category);
  const [itemSlot, setItemSlot] = useState<EquipmentSlot>(item.slot);
  const [properties, setProperties] = useState<string[]>(() => item.properties.filter((tag) => !(tag in ROUTED_SHEET_EFFECTS)));
  const [routedEffects, setRoutedEffects] = useState<string[]>(() => item.properties.filter((tag) => tag in ROUTED_SHEET_EFFECTS));
  const [confirmingDelete, setConfirmingDelete] = useState(false);
  const resetDraft = () => {
    setName(item.name);
    setHighlight(item.summary);
    setDescription(item.mechanics);
    setItemCategory(item.category);
    setItemSlot(item.slot);
    setProperties(item.properties.filter((tag) => !(tag in ROUTED_SHEET_EFFECTS)));
    setRoutedEffects(item.properties.filter((tag) => tag in ROUTED_SHEET_EFFECTS));
  };
  const save = () => {
    if (!name.trim()) return;
    onSave({ name: name.trim(), summary: highlight.trim(), mechanics: description.trim(), category: itemCategory, slot: itemSlot, properties: [...properties, ...routedEffects] });
    setIsEditing(false);
  };
  const requestDelete = () => { resetDraft(); setIsEditing(false); setConfirmingDelete(true); };
  return <>
    <button type="button" onClick={() => setIsEditing((shown) => !shown)} className="rounded-lg bg-slate-700 px-3 py-2 text-xs font-bold text-white">{isEditing ? 'Close Edit' : 'Edit'}</button>
    <button type="button" onClick={requestDelete} className="rounded-lg px-3 py-2 text-xs font-bold text-red-300">Delete</button>
    {isEditing && <div className="mt-3 w-full rounded-xl border border-violet-400/20 bg-violet-500/5 p-3">
      <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500">Name<input value={name} onChange={(event) => setName(event.target.value)} className={`${fieldClass} mt-1`} /></label>
      <label className="mt-3 block text-[10px] font-bold uppercase tracking-wider text-slate-500">Item Highlight (optional)<input value={highlight} onChange={(event) => setHighlight(event.target.value)} className={`${fieldClass} mt-1`} placeholder="A short standout detail…" /></label>
      <label className="mt-3 block text-[10px] font-bold uppercase tracking-wider text-slate-500">Description<textarea value={description} onChange={(event) => setDescription(event.target.value)} className={`${fieldClass} mt-1 min-h-24 resize-y`} /></label>
      <div className="mt-3 grid grid-cols-2 gap-3">
        <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500">Category<select value={itemCategory} onChange={(event) => setItemCategory(event.target.value as EquipmentCategory)} className={`${fieldClass} mt-1 w-full`}>{Object.values(EquipmentCategoryValues).map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
        <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500">Slot<select value={itemSlot} onChange={(event) => setItemSlot(event.target.value as EquipmentSlot)} className={`${fieldClass} mt-1 w-full`}>{Object.values(EquipmentSlotValues).map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
      </div>
      <PillMultiSelect label="Properties" hint="Some Properties apply automatic effects — e.g. Weapons: Guard; Spell Focuses: Channeling, Vicious, Powerful, Protective, Warded." options={propertyOptions} selected={properties} onToggle={(value) => setProperties((current) => toggleValue(current, value))} />
      <PillMultiSelect label="Routed-Character Sheet Effects" hint="Applies while the item is equipped, no matter its Category." options={routedEffectOptions} selected={routedEffects} onToggle={(value) => setRoutedEffects((current) => toggleValue(current, value))} tone="emerald" />
      <div className="mt-3 flex justify-end gap-2"><button type="button" onClick={() => { resetDraft(); setIsEditing(false); }} className="rounded-lg bg-slate-800 px-3 py-2 text-xs font-bold text-slate-300">Cancel</button><button type="button" disabled={!name.trim()} onClick={save} className="rounded-lg bg-violet-700 px-3 py-2 text-xs font-black text-white hover:bg-violet-600 disabled:cursor-not-allowed disabled:opacity-35">Save Changes</button></div>
    </div>}
    {confirmingDelete && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true" aria-label={`Delete ${item.name}`}>
      <div className="w-full max-w-sm rounded-2xl border border-violet-400/20 bg-slate-900 p-5 shadow-2xl">
        <h3 className="font-black text-slate-100">Delete {item.name}?</h3>
        <p className="mt-2 text-sm text-slate-400">You can permanently delete this custom item everywhere it's used, or just remove it from this character while keeping it in the Equipment list for future use.</p>
        <div className="mt-4 flex flex-col gap-2">
          <button type="button" onClick={() => { onDeletePermanently(); setConfirmingDelete(false); }} className="rounded-lg bg-red-700 px-3 py-2 text-xs font-black text-white hover:bg-red-600">Permanently Delete</button>
          <button type="button" onClick={() => { onRemoveFromCharacter(); setConfirmingDelete(false); }} className="rounded-lg bg-violet-700 px-3 py-2 text-xs font-black text-white hover:bg-violet-600">Add to Equipment List for Future Use</button>
          <button type="button" onClick={() => setConfirmingDelete(false)} className="rounded-lg bg-slate-800 px-3 py-2 text-xs font-bold text-slate-300">Cancel</button>
        </div>
      </div>
    </div>}
  </>;
...
function EquipmentTab({ character, equipmentCatalog, onChange, onRoll }: { character: Character; equipmentCatalog: EquipmentCatalogItem[]; onChange: CharacterSheetTabContentProps['onChange']; onRoll: CharacterSheetTabContentProps['onRoll'] }) {
  ...
  const [customItemHighlight, setCustomItemHighlight] = useState('');
  const [customItemDescription, setCustomItemDescription] = useState('');
  const [customItemCategory, setCustomItemCategory] = useState<EquipmentCategory>(EquipmentCategoryValues.ADVENTURING_SUPPLIES);
  const [customItemSlot, setCustomItemSlot] = useState<EquipmentSlot>(EquipmentSlotValues.CARRIED);
  const [customItemProperties, setCustomItemProperties] = useState<string[]>([]);
  const [customItemRoutedEffects, setCustomItemRoutedEffects] = useState<string[]>([]);
  const customEquipmentList = useCampaignStore((state) => state.campaignData.customEquipment);
  ...
  )).sort((a, b) => a.localeCompare(b)), [equipmentCatalog, customItemIDs]);
  const routedEffectOptions = Object.keys(ROUTED_SHEET_EFFECTS);
  ...
  const filtered = useMemo(() => { const query = search.trim().toLowerCase(); return equipmentCatalog.filter((item) => (category === 'All' || (category === 'Custom Items' ? customItemIDs.has(item.id) : item.category === category)) && (!query || [item.name, item.subtype, item.summary, ...item.properties].some((value) => value.toLowerCase().includes(query)))); }, [category, customItemIDs, equipmentCatalog, search]);
  ...
  const createCustomItem = () => {
    ...
    const item: EquipmentCatalogItem = {
      ...
      subtype: 'Custom Item',
      summary: customItemHighlight.trim(),
      mechanics: customItemDescription.trim(),
      properties: [...customItemProperties, ...customItemRoutedEffects],
      slot: customItemSlot,
    ...
    addItem(item);
    setCustomItemName('');
    setCustomItemHighlight('');
    setCustomItemDescription('');
    setCustomItemCategory(EquipmentCategoryValues.ADVENTURING_SUPPLIES);
    setCustomItemSlot(EquipmentSlotValues.CARRIED);
    setCustomItemProperties([]);
    setCustomItemRoutedEffects([]);
    setShowCustomItemForm(false);
  };
  const drinkPotion = (entry: CharacterInventoryItem, item: EquipmentCatalogItem) => {
    const amount = healingPotionAmount(item);
    const useCapacity = equipmentUseCapacity(item);
    if (useCapacity !== undefined && (entry.remainingUses ?? entry.quantity * useCapacity) <= 0) {
      setNotice(`${item.name} has no uses remaining.`);
      return;
    }
    const healthPoints = Math.min(character.maxHealthPoints, character.healthPoints + amount);
    onChange({
      healthPoints,
      inventoryItems: useCapacity === undefined
        ? consumeInventoryQuantity(inventory, entry.id)
        : spendInventoryUse(inventory, entry.id, useCapacity),
    });
  ...
  const toggleGear = (entry: CharacterInventoryItem, item: EquipmentCatalogItem) => {
    ...
    onChange({
      currentAP: character.currentAP - actionPointCost,
      inventoryItems: nextInventory.map((candidate) => candidate.id === entry.id && !entry.isEquipped && item.requiresAttunement
        ? { ...candidate, isAttuned: true }
        : candidate),
    });
...
function createCompanion(character: Character, kind: CharacterCompanionKind, spell?: Spell): CharacterCompanion {
  ...
  return {
    ...
    saveDC: familiar || summon ? character.saveDC ?? 10 + character.primeModifier + character.combatMastery : 10,
    attributes: companionAttributes(character, source),
    features: [familiar ? FAMILIAR_RULES : summon ? SUMMON_RULES : '', spellRules].filter(Boolean).join('\n\nSource Spell\n'),
    notes: '',
  };
}
