...
function WeaponAttackCard({ item, modifier, adjustment, damageBonus, trained, prone, canUseVersatileTwoHanded, canReload, availableAP, availableSP, onReload, onSpendEnhancements, onThrown, onRoll }: {
  ...
  const activeRange = isRangedAttack ? (profile?.isNativeRanged ? profile.range : profile?.thrownRange) : item.properties.includes('Reach') ? '2' : '1';
  ...
  const heavyHitDamage = hitDamage + 1 + (profile?.heavyHitDamageBonus ?? 0);
  const brutalHitDamage = hitDamage + 2 + (profile?.heavyHitDamageBonus ?? 0);
  ...
  const rollAttack = () => {
    ...
    onRoll(`${item.name} Martial Attack • ${hitDamage} ${damageType} damage${accuracyDie ? ` • Accuracy d4: ${accuracyDie}` : ''}${enhancements ? ` • ${enhancements}` : ''}`, displayedModifier + accuracyDie, situational);
...
function CombatTab({ character, training, modifiers, equipmentCatalog, knownSpells, knownManeuvers, grantedSpells, grantedManeuvers, ancestryGrantedSpells, selectedTraits, onChange, onRoll }: {
  ...
  return <div className="space-y-5">
    <section className={panelClass}><SectionHeading eyebrow="At a Glance" title="Combat Reference" /><div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">{referenceStats.map(([label, value, rollModifier]) => rollModifier === null ? <div key={label} className="rounded-lg bg-slate-950/55 p-3"><div className="text-xs text-slate-500">{label}</div><div className="text-xl font-black text-slate-100">{value}</div>{(label === 'Physical Defense' || label === 'Area Defense') && <div className="mt-1 text-[10px] font-bold text-amber-200">Heavy {Number(value) + 5} • Brutal {Number(value) + 10}</div>}{label === 'Physical Defense' && physicalDefense !== character.physicalDefense && <div className="text-[10px] font-bold text-red-300">Active feature adjustment</div>}{label === 'Area Defense' && areaDefense !== character.arcaneDefense && <div className="text-[10px] font-bold text-red-300">Active feature adjustment</div>}{label === 'Speed' && shellRetreatActive && <div className="text-[10px] font-bold text-rose-300">Shell Retreat: cannot move</div>}</div> : <button type="button" key={label} onClick={() => onRoll(label, rollModifier, (label === 'Martial Check' ? 0 : modifiers.attackAndSpellDisadvantage) - Number(label === 'Spell Attack' && prone))} className="rounded-lg bg-slate-950/55 p-3 text-left hover:bg-violet-500/10"><div className="text-xs text-slate-500">{label}</div><div className="text-xl font-black text-slate-100">{value}</div><div className="text-xs font-bold text-violet-300">Roll</div>{label !== 'Martial Check' && modifiers.attackAndSpellDisadvantage < 0 && <div className="text-[10px] font-bold text-rose-300">{Math.abs(modifiers.attackAndSpellDisadvantage)}× gear DisADV</div>}{label === 'Spell Attack' && prone && <div className="text-[10px] font-bold text-rose-300">Prone DisADV</div>}</button>)}</div>{uniqueStrings([...effects.resistances, ...ancestryMechanics.resistances]).length > 0 && <div className="mt-3 rounded-lg border border-sky-400/15 bg-sky-500/5 p-2 text-xs text-sky-100"><strong>Active Resistances:</strong> {uniqueStrings([...effects.resistances, ...ancestryMechanics.resistances]).join(' • ')}</div>}{ancestryMechanics.vulnerabilities.length > 0 && <div className="mt-3 rounded-lg border border-rose-400/15 bg-rose-500/5 p-2 text-xs text-rose-100"><strong>Vulnerabilities:</strong> {ancestryMechanics.vulnerabilities.join(' • ')}</div>}{ancestryMechanics.defenses.length > 0 && <div className="mt-3 rounded-lg border border-emerald-400/15 bg-emerald-500/5 p-2 text-xs text-emerald-100"><strong>Ancestry Defenses:</strong> {ancestryMechanics.defenses.join(' • ')}</div>}{ancestryMechanics.senses.length > 0 && <div className="mt-3 rounded-lg border border-violet-400/15 bg-violet-500/5 p-2 text-xs text-violet-100"><strong>Senses:</strong> {ancestryMechanics.senses.join(' • ')}</div>}{ancestryMechanics.movement.length > 0 && <div className="mt-3 rounded-lg border border-amber-400/15 bg-amber-500/5 p-2 text-xs text-amber-100"><strong>Movement:</strong> {ancestryMechanics.movement.join(' • ')}</div>}{shellRetreatActive && <div className="mt-3 rounded-lg border border-emerald-400/20 bg-emerald-500/10 p-3 text-xs leading-5 text-emerald-100"><strong>Shell Retreat active:</strong> Physical Damage Reduction, Elemental Damage Reduction, ADV on Might Saves; you are Prone, cannot move, have DisADV on Agility Saves, and cannot take Reactions.</div>}{(modifiers.spellCheckBonus > 0 || modifiers.spellAttackBonus > 0 || modifiers.spellAttackDamageBonus > 0) && <div className="mt-3 rounded-lg border border-fuchsia-400/15 bg-fuchsia-500/5 p-2 text-xs text-fuchsia-100"><strong>Equipped Spell Focus:</strong> {[modifiers.spellCheckBonus > 0 ? `+${modifiers.spellCheckBonus} Spell Checks` : '', modifiers.spellAttackBonus > 0 ? `+${modifiers.spellAttackBonus} Spell Attacks` : '', modifiers.spellAttackDamageBonus > 0 ? `+${modifiers.spellAttackDamageBonus} Spell Attack damage` : ''].filter(Boolean).join(' • ')}</div>}<EquippedRulesSummary modifiers={modifiers} /></section>
    ...
    <section className={panelClass}><div className="mb-3 flex flex-wrap items-end justify-between gap-2"><SectionHeading eyebrow="Equipped Arsenal" title="Martial Attacks" tone="text-amber-300" /><span className="text-xs text-slate-500">Published damage, ranges, properties, and situational penalties are applied here.</span></div><div className="grid gap-3 lg:grid-cols-2"><UnarmedAttackCard modifier={martialCheck} adjustment={modifiers.attackAndSpellDisadvantage} damageBonus={effects.martialMeleeDamageBonus} impactBonus={modifiers.unarmedHeavyHitDamageBonus} prone={prone} onRoll={onRoll} /><NaturalWeaponAttackCard character={character} traits={selectedTraits} onChange={onChange} onRoll={onRoll} />{equippedWeapons.map(({ inventory, item }) => <WeaponAttackCard key={inventory.id} item={item} modifier={martialCheck} adjustment={modifiers.attackAndSpellDisadvantage} damageBonus={effects.martialMeleeDamageBonus} trained={training.weaponTraining || training.pactWeaponTraining} prone={prone} canUseVersatileTwoHanded={canUseVersatileTwoHanded(inventory.id)} canReload={canUseVersatileTwoHanded(inventory.id)} availableAP={character.currentAP} availableSP={character.stamina} onReload={(resource) => onChange(resource === 'AP' ? { currentAP: character.currentAP - 1 } : { stamina: character.stamina - 1 })} onSpendEnhancements={(resource, amount, weaponWasThrown) => onChange({ ...(resource === 'AP' ? { currentAP: character.currentAP - amount } : { stamina: character.stamina - amount }), ...(weaponWasThrown ? { inventoryItems: (character.inventoryItems ?? []).map((entry) => entry.id === inventory.id ? { ...entry, isEquipped: false } : entry) } : {}) })} onThrown={() => stowThrownItem(inventory.id)} onRoll={onRoll} />)}{equippedShields.map(({ inventory, item }) => <ShieldAttackCard key={`shield-${inventory.id}`} item={item} modifier={martialCheck} adjustment={modifiers.attackAndSpellDisadvantage} trained={item.subtype === 'Heavy Shield' ? training.heavyShieldTraining : training.lightShieldTraining} prone={prone} onThrown={() => stowThrownItem(inventory.id)} onRoll={onRoll} />)}{equippedNets.map(({ inventory }) => <NetAttackCard key={`net-${inventory.id}`} character={character} modifier={martialCheck} onThrow={() => stowThrownItem(inventory.id, 1)} onRoll={onRoll} />)}</div>{equippedWeapons.length === 0 && equippedShields.length === 0 && equippedNets.length === 0 && !selectedTraits.some(({ name }) => name === 'Natural Weapon') && <p className="mt-3 text-center text-xs text-slate-500">Equip a weapon, shield, or Net to add its attack controls.</p>}</section>
