...
function ChecksTab({ character, reference, equipmentCatalog, equipmentModifiers, selectedTraits, onRoll }: {
  ...
  const skillModifier = (name: string, mastery: MasteryLevel) => {
    ...
    const wildFormIncrease = wildForm.active && wildForm.skillMasteries.includes(name) && baseRank < skillMasteryCap(character) ? 1 : 0;
    return attribute + masteryBonus(masteryTitle(baseRank + (expertise.skills[name] ?? 0) + wildFormIncrease));
  };
  const effectiveSkillMastery = (name: string, mastery: MasteryLevel) => {
    const baseRank = masteryRank(mastery);
    const increase = wildForm.active && wildForm.skillMasteries.includes(name) && baseRank < skillMasteryCap(character) ? 1 : 0;
    return masteryTitle(baseRank + (expertise.skills[name] ?? 0) + increase);
  };
  const skillAdjustment = (name: string) => {
    ...
    return skillfulAdvantage + gearAdjustment;
  };
